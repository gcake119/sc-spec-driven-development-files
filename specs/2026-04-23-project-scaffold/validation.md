...
- `npm test` runs Vitest validation tests and includes at least one passing smoke test.
- `npm run test:validation` runs the same Vitest validation suite successfully.
...
- Home layout remains usable and readable at 320px, 768px, and 1024px+ viewport widths without horizontal overflow.
...
- Responsive style rules are present for mobile, tablet, and desktop breakpoints.
...
- Responsive regressions are checked as part of merge readiness.
